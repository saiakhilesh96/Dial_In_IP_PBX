...
//            document.getElementById(sid).innerHTML = "<input type=button value=Update onclick=ajax('" + user_id + "','" + phone_number + "');>"; // Add different color to background
    ...
    <body>
        ...
        header('Content-disposition: attachment; filename= WhiteList_Contacts.xls');
        echo $_POST["data"];
        echo "<div id=\"msgDsp\" STYLE=\" FONT-SIZE: 12px;font-family: Verdana;border-style: solid;border-width: 1px;border-color:white;padding:0px;height:20px;width:250px;top:10px;z-index:1\">  </div>";
        $userid = $_POST['user_id'];
        $phonenumber = $_POST['phonenumber'];
        $bool = 0;
        $wlc_qry = "SELECT * FROM WHITELIST_CONTACTS";
        if ($userid != null || $phonenumber != null) {
            $wlc_qry = $wlc_qry . " where";
        }
        if ($userid != null) {
            $wlc_qry = $wlc_qry . " user_id like '%" . $userid . "%'";
            $bool = 1;
        }
        if ($phonenumber != null) {
            if ($bool) {
            ...
            $wlc_qry = $wlc_qry . " phone_number like '%" . $phonenumber . "%'";
        }
        //$qry = $qry . " order by start_time desc";
        $result = mysqli_query($conn, $wlc_qry);
        //$wlp = $dbo->prepare($wlc_qry);
...
//                . "<th style= 'width:0.5em; text-align: center'>I-Prefix</th>"
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $calltypeqry = "SELECT calltype_name,o_prefix,i_prefix FROM CALL_TYPE WHERE calltype_id= " . $row['calltype_id'];
                $Rs = mysqli_query($conn, $calltypeqry);
                $calltypearray = mysqli_fetch_array($Rs);
                $calltype = $calltypearray['calltype_name'];
                $oprefix = $calltypearray['o_prefix'];
                $iprefix = $calltypearray['i_prefix'];
                ...
                . "<td style= 'width:3em;'><div id=$username_id>$name</div></td>"
                . "<td style= 'width:3em;'><div id=$usercourse_id>$course</div> </td>"
                ...
                . " <td style= 'width:0.5em;'><div id=$iprefix_id>$iprefix</div></td>"
                . " <td style= 'width:0.5em;'><div id=$oprefix_id>$oprefix</div></td>"
                . " <td style= 'width:3em;'><div id=$calltypename_id>$calltype</div><div id=$calltype_id style=\"visibility:hidden\">$row[calltype_id]</div></td>"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <input name= "start" class='form-inline fromdate' size= '12' type=text id="startdate" placeholder="Enter Start Date">
                                <input name= "end" class='form-inline todate' size= '12' type=text id="enddate" placeholder="Enter End Date">
                            </div>
